...
public class OfferVoucherController {
    ...
    @ApiImplicitParams({
            @ApiImplicitParam( name = "id",value = "id(修改时传)", required = false, dataType = "String"),
            @ApiImplicitParam( name = "type",value = "优惠类型(打折：discount|满减：full_cut|满折:full_discount)", required = true, dataType = "String"),
            @ApiImplicitParam( name = "full",value = "满多少", required = true, dataType = "double"),
            @ApiImplicitParam( name = "discount",value = "折扣", required = false, dataType = "double"),
            @ApiImplicitParam( name = "less",value = "减多少", required = false, dataType = "double"),
            @ApiImplicitParam( name = "imageUrl", value = "优惠图", required = false, dataType = "String"),
            @ApiImplicitParam( name = "startTime", value = "开始时间", required = false, dataType = "String"),
            @ApiImplicitParam( name = "endTime",value = "结束时间", required = false, dataType = "String")
    })
    @PostMapping(value = "addOrUpdate")
    public JSONObject addOrUpdate(@Valid OfferVoucher offerVoucher) {
        RestMessage restMessage = offerVoucherService.addOrUpdate(offerVoucher);
        return JSONObjResult.toJSONObj(restMessage);
    }

    @ApiOperation(value = "结束")
    @PostMapping(value = "over")
    public JSONObject over(String id) {
        String logMsg = "结束优惠活动";
        LogbackLogger.info(logMsg+id);
        if(StringUtils.isEmpty(id)){
            throw new CustomException("主键不能为空!");
        }
        RestMessage restMessage = offerVoucherService.over(id);
        return JSONObjResult.toJSONObj(restMessage);
    }
}
