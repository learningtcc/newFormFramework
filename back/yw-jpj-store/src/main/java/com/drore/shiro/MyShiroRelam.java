...
public class MyShiroRelam extends AuthorizingRealm {
    ...
    protected AuthorizationInfo doGetAuthorizationInfo(
        ...
        if (ucUserInfo != null) {
            ...
            //menus = new UcBuilderImpl().getUcTwoMenuByToken(ucUserInfo.getToken(), DefaultProperties.getValueByKey(DefaultProperties.systemCode));
            if (menus != null)
    ...
    protected AuthenticationInfo doGetAuthenticationInfo(
        ...
        // 查出是否有此用户
        StoreUser storeUser = storeUserService.queryUserName(token.getUsername());
        ...
        // 获取密码盐值 系统存的
        String pswdSalt = storeUser.getSalt();
        // 用户填写的登录密码
        ...
        if (validatePassWord.equals(sysPassWord)) {// 如果密码正确
            ...
            //增加密码失效判断时间
            if(CommonEnum.YesOrNo.YES.getCode().equalsIgnoreCase(storeUser.getIsResetPassword())){//如果是重置密码，
                Date invalidTime = storeUser.getPasswordInvalidTime();//获取密码失效时间
            ...
            //判断是否锁定
            String isLock = storeUser.getIsLock();
            if (CommonEnum.YesOrNo.YES.getCode().equalsIgnoreCase(isLock)) {// 如果为0表示锁定
                ...
                return new SimpleAuthenticationInfo(storeUser, sysPassWord,
                        ByteSource.Util.bytes(storeUser.getSalt()), getName());
            }
        } else
            throw new UnknownAccountException("账号或者密码不正确");// 没找到帐号
