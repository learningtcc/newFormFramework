...
export default function interactCommentForm(opt) {
	var fields = [{
		column: [{
			label: '互动主题',
			readonly: true,
		...
		edit: {
			...
			events: {
				click: function(opt) {
					...
					if (opt.panel.items['interactCommentForm'].validatesFn({windowBtn: opt.btn})) {
            			var loading = layer.load(1, { shade: [0.1, '#fff'] })
            			console.log($('.' + opt.panel.component.className.split(' ')[0]).find('form').serialize())
            			$.ajax({
				            url: '/auditManagement/updateInteractiveEvaluationAudit',
				            type: 'post',
	...
	return new WindowPanel({
		...
		items: [
			setConfig({
				opt: opt,
				fnName: 'interactCommentForm',
				config: {
					xtype: 'FormPanel',
					data: opt.data,
					fields: fields,
					queryUrl: '/auditManagement/queryById',
					queryType: 'get',
					render: (opt) => {
