...
public class SysAuthorityController {
	...
	public JSONObject removeById(@RequestParam String id) {
		...
		try {
			...
			// 如果不存在则返回
			if (!sysAuthorityService.checkIdExist(id )) {
				return JSONObjResult.toJSONObj("权限不存在");
			}
			RestMessageModel model = sysAuthorityService.deleteById(id );
            if(model.isSuccess())
            	//删除缓存
	...
	public JSONObject save(SysAuthority sysAuthority) {
		...
		try {
			...
			RestMessageModel restMessageModel = sysAuthorityService
					.save(sysAuthority);
	...
	public JSONObject update(SysAuthority sysAuthority) {
		...
		try {
			if (null == sysAuthority
					|| StringUtils.isEmpty(sysAuthority.getId())) {
				return JSONObjResult.toJSONObj("主键不能为空");
			}
			// 如果不存在则返回
			if (!sysAuthorityService.checkIdExist(sysAuthority.getId() )) {
				return JSONObjResult.toJSONObj("权限不存在");
			}
			...
			if (CommonConstant.ISNOTENABLE.equals(isEnable)) {
				...
				if (!CommonConstant.ROOT.equals(sysAuthority.getParentId())) {
					SysAuthority queryOne = sysAuthorityService.queryOne(
							sysAuthority.getParentId());
	...
	public JSONObject queryByParentId(String parentId) {
		...
		try {
			...
			if (!CommonConstant.ROOT.equals(parentId)
					&& !sysAuthorityService.checkIdExist(parentId )) {
	...
	public JSONObject queryAllAuthorityByEnable(@RequestParam String enable) {
		log.info("查询所有可用角色");
		try {
			List<SysAuthority> authoritys = sysAuthorityService
					.queryAll(enable);
	...
	public JSONObject queryTree(String enable) {
		...
		try {
            ...
            if(authoritys==null || authoritys.size()==0){
				authoritys =sysAuthorityService. queryAllChildren(CommonConstant.ROOT,
